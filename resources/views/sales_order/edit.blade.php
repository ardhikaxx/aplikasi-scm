...
@section('content')
    ...
    <form action="{{ route('sales_order.update', $salesOrder->id_so) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" name="tanggal" id="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', $salesOrder->tanggal) }}">
            @error('tanggal')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nama_customer" id="nama_customer" class="form-control @error('nama_customer') is-invalid @enderror" value="{{ old('nama_customer', $salesOrder->nama_customer) }}">
            @error('nama_customer')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
